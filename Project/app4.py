...
# from routes.camera_routes import camera_bp  # 실제 파일이 준비되면 이 줄의 주석을 풀고 아래 더미 코드를 삭제합니다.
# from routes.ai_routes import ai_bp        # 실제 파일이 준비되면 이 줄의 주석을 풀고 아래 더미 코드를 삭제합니다.


# --- 임시 Blueprint 정의 (BuildError 방지용) ---
# 나중에 실제 기능이 구현된 .py 파일로 옮겨야 합니다.
camera_bp = Blueprint('camera', __name__, url_prefix='/camera')
@camera_bp.route('/')
def show_camera():
    # 실제 카메라 페이지가 만들어지면 이 부분을 수정합니다.
    # 지금은 임시로 텍스트만 반환하여 오류를 방지합니다.
    return render_template('camera.html') # templates/camera.html 파일 필요

ai_bp = Blueprint('ai', __name__, url_prefix='/ai')
@ai_bp.route('/')
def show_ai_analysis():
    return render_template('ai.html') # templates/ai.html 파일 필요
            ...
            if not manual_override['LED']:
                ...
                if current_on_time < target_light_seconds:
                    ...
                    if device_state[pins['LED']] == GPIO.LOW:
                        ...
                        db_update = get_db_connection(); cursor_update = db_update.cursor()
                        cursor_update.execute("UPDATE control_state SET light_on_seconds_acc = %s, light_last_update_time = %s LIMIT 1", (int(final_acc_seconds), now))
                        db_update.commit(); cursor_update.close(); db_update.close()
        ...
        for pin_str, value_str in request.form.items():
            ...
            if device_state[pin_num] != new_state:
                ...
                if device_name == 'LED':
                    ...
                    last_update = led_timer_data['last_update_time']
...
app.register_blueprint(camera_bp) # 임시 Blueprint 등록
app.register_blueprint(ai_bp)       # 임시 Blueprint 등록
